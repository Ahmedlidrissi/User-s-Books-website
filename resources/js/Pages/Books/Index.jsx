...
function Index(){
    ...
    return(
        <>
        <Link className="add-book" href='/books/create'>Add a new book</Link>
        {books && books.length > 0 ? books.map(book=>(
            <div className="Card-container">
                <img className='book_img' src={book.cover_image} alt="Book" />
                <Link href={`/books/${book.id}`}><h1>{book.title} </h1></Link>
                <h3>{book.author}</h3>
